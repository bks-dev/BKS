        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    CFund::CProposal proposal;
                    if(CFund::FindProposal(prequest.proposalhash, proposal)) {
                        CBKSAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
...
void BKSStaker(const CChainParams& chainparams)
{
    LogPrintf("BKSStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BKS-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in BKSStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running BKSStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BKSStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BKSStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("BKSStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
