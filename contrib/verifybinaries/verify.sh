...
###   This script attempts to download the signature file SHA256SUMS.asc from BKS.org
...
RCSUBDIR="test/"
BASEDIR="https://BKS.org/bin/"
VERSIONPREFIX="BKS-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'BKS-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BKS Core binary release signing key installed?"
   fi
