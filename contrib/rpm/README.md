...
The RPM spec file provided here is for BKS-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://BKS.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BKS git repository](https://github.com/BKS/BKS) but are not part of
...
Currently only `Source10` is used, for the example `BKS.conf` file.
...
in the BKS git repository. At present only an SVG version of the BKS
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the BKS
developers so that an official fix to the issue can make it into the next
release.

### Patch0 BKS-0.12.0-libressl.patch
...
needed when 0.12.1 is released, a proper fix is already in the BKS git
master branch.
...
idea to build BKS Core against a static version of that library compiled
...
The `BKS.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `BKS-server` package. At this point, most RPM based
...
The `BKS-server` package should have SELinux support. How to properly do
...
BKS-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/BKS/). He has been
packaging BKS for Fedora at least since 2012.
...
4. I renamed the `BKS` package that contains the Qt GUI to `BKS-core` as
that appears to be how the general population refers to it, in contrast to
`BKS-xt` or `BKS-classic`. I wanted to make sure the general population
